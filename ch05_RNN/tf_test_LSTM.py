...
# extract the last relevant outputand use in a linear layer
final_output = tf.matmul(states[1],
                         weights['linear_layer']) + bias['linear_layer']
...
with tf.Session() as sess:
    ...
    for step in range(1000):
        x_batch, y_batch, seqlen_batch = get_sentence_batch(batch_size,
                                                            ...
                                                            train_y,
                                                            train_seqlens)
        sess.run(train_step, feed_dict={_inputs: x_batch, _labels: y_batch, _seqlen: seqlen_batch})

        if step % 100 == 0:
            acc = sess.run(accuracy, feed_dict={_inputs: x_batch, _labels: y_batch, _seqlen: seqlen_batch})
            print(f'Train accuracy at {step} : {acc}')
    ...
    for test_batch in range(5):
        ...
        batch_pred, batch_acc = sess.run(
            ...
            feed_dict={_inputs: x_test, _labels: y_test, _seqlen: seqlen_test}
        )
        print(f'Test accuracy {test_batch} : {batch_acc}')
